    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        #             1, (0, 0, 0), 2)
            ...
            if (size%2==0):
                ...
                print(math.sqrt((bx[size-1]-bx[size-2])*(bx[size-1]-bx[size-2])+(by[size-1]-by[size-2])*(by[size-1]-by[size-2]))/(273/int(scale))    )
                # /10
                #messagebox.showinfo("Distance","Distance from"+ str(size-1) +"to"+ str(size)+  "="+ str(f)+" cm")
                rect=cv2.circle(resized_down,(ox,oy),f,(0,0,0),2)
                s=f*f*3.14
                s=int(s)
                cv2.putText(resized_down,"S="+str(s/100)+"cm^2",(x,y),font,1,(0,0,0),2)
        if (shape=="Rectangle"):
            scale=(cb_diameter.get())
            if (scale==''):
                print("sai")
            ...
            if (size%4==0):
                ...
                # /10
                #messagebox.showinfo("Distance","Distance from"+ str(size-1) +"to"+ str(size)+  "="+ str(f)+" cm")
                rect=cv2.rectangle(resized_down,(ox,oy),f,(0,0,0),2)
                s=f*f*3.14
                s=int(s)
                cv2.putText(resized_down,"S="+str(s/100)+"cm^2",(x,y),font,1,(0,0,0),2)
        cv2.imshow('image', resized_down)
