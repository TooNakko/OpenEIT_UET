...
`Data.txt` in general is a file containing numbers seperating by `,`, each line has the same numbers of variable.

The length of data in each line in `data.txt` must match the column of the `h_matrix` (the second element).

The more numbers of electrode, the more data it is allowed to use (better resolution).
