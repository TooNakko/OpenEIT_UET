...
class Forward(object):
    ...
    def solve_eit(self, ex_mat=None, step=1, perm=None, parser=None):
        ...
        for i in range(n_lines):
            ...
            f_el = f[self.el_pos]
            #print(perm0) matrix perm though we setup to control  
            #print(f_el)
            #print(f)
            #print(ex_line) pair subtract vol
            #print(jac_i)           
    ...
    def solve(self, ex_line, perm):
        ...
        #print(ke)#-> matrix 3x3 matrix combine t

        # 2. assemble to global K
        kg = assemble_sparse(ke, self.tri, perm, self.n_pts, ref=self.ref)
        #print(perm) #matrix 1x number_point
        #print(kg)
        #print(self.n_pts) -> number of element
        #print(self.ref) ->16
        # 3. calculate electrode impedance matrix R = K^{-1}
        r_matrix = la.inv(kg)
        #print((r_matrix)) #-> value of node
...
def calculate_ke(pts, tri):
    ...
    #print(ke_array) create new zero matrix
    #print(n_tri)
    for ei in range(n_tri):
        no = tri[ei, :]
        xy = pts[no]
        #print(xy) #coordinate xy of peak element... matrix 3x2
        #print(no) #matrix 1x3 peak of element
        
        #print(" ")
        # compute the KIJ (permittivity=1.0)
        ke = _k_local(xy)
        #print(ke) matrix 3x3
        #print(len(ke))
        ke_array[ei] = ke
...
def _k_triangle(xy):
    ...
    # s2 = xy[0, :] - xy[2, :]
    # s3 = xy[1, :] - xy[0, :]
    #print(s) matrix 3x2, coordinate of 3 edges of triangle
    ...
    #print(ke_matrix) matrix 3x3 -> stiffness matrix
    return ke_matrix
